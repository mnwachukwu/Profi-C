...
| [throwaway.pc](samples/throwaway.pc) | **A name for the value you do not want.** A bare `_` in a loop that only counts, a walk that ignores its element, a `catch` that goes by type alone, and a result worked out and dropped — several in one body, and none of them readable |
...
| [throwaway.pc](samples/negatives/compile/throwaway.pc) | A `_` read back, handed on, left with no value to drop, and used to name a field, a function, a parameter, a type and a namespace — beside a local nothing reads |
