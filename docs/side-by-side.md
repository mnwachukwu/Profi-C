...
| **Numbers** | `integer` (64-bit), `real`, `fraction` | `byte` through `ulong`, `float`, `double`, `decimal`, `BigInteger` | One whole-number type means no width to choose and no unsigned surprises. It also means no `decimal` for money and no wider-than-64 arithmetic |
